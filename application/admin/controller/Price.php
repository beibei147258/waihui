...
class Price extends Base
{
	...
	public function pricelist()
	{
		...
		$list = db('userinfo')->field('uid,username,utel,oid,managername,usermoney,nickname')
		...
		foreach ($list as $key => $v) {
			...
			$_list[$key]['all_res'] = $price_log->where($all_res_map)->sum('account');
			//入金次数 
			$_list[$key]['all_res_count'] = $price_log->where($all_res_map)->count();
			//出金总额
			...
			$_list[$key]['all_cash'] = $price_log->where($all_cash_map)->sum('account');
			//出金次数
			$_list[$key]['all_cash_count'] = $price_log->where($all_cash_map)->count();
			//出金审核中
			//$all_cash_map['isverified'] = 0;
			$_list[$key]['all_cash_shenhe'] = $price_log->where($all_cash_map)->sum('account');
			//佣金
			$yj_map['uid'] = $v['uid'];
			$yj_map['title'] = '客户手续费';
			if(isset($getdata['starttime']) && !empty($getdata['starttime'])){
			...
			$_list[$key]['all_cash_shoudong'] = $price_log->where($all_cash_map)->sum('account');
			//实际余额：
	...
	public function myprice()
	{
		...
		$list = db('price_log')->alias('pl')->field('pl.*,u.uid,u.username,u.nickname,u.utel')
				->join('__USERINFO__ u','u.uid=pl.uid')
				->where($map)->order('id desc')->paginate($pagenum,false,['query'=> $getdata]);;
	...
	public function pricesta()
	{
        ...
        $all_res_map['type'] = 1;
		$_list['all_res'] = $price_log->where($all_res_map)->sum('account');
        ...
        $all_cash_map['type'] = 2;
		$_list['all_cash'] = $price_log->where($all_cash_map)->sum('account');
