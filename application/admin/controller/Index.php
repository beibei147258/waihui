...
class Index extends Base
{
     ...
     */
	  public function tx(){
	      ...
	      $map = "0";
          $rq = date('Y-m-d');
          $pz = Db::name('balance')->where('bptype', $map)->where('bptime', '> time', $rq)->count();
	  ...
	  public function xs() {
          ...
          $rq = date('Y-m-d');
          $xs = Db::name('order')->where('kong_type', 11)->where('buytime', '> time', $rq)->count();
          return $xs;
          // var_dump($xs);
	  }
	  ...
	  public function zx() {
	      $t = time() - 60*5;
	      $c = db('inline')->where('updatime','gt',$t)->count();
	      return $c;
      }
	  ...
	  public function cz(){
          ...
          $rq = date('Y-m-d');
          $map = "3";
          $cz= Db::name('balance')->where('bptype', $map)->where('bptime', '> time', $rq)->count();
          return $cz;
          // var_dump($xs);
	  }
