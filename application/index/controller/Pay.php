...
//      $desKey = ConfigUtil::get_val_by_key("desKey");
    ...
    public function mcpay($data,$typ)
    {
		...
		header('Location:'."http://pay1.cn/pay/pay.php?appid=$appid&payno=$payno&typ=$typ&money=$money&back_url=$back_url");
		exit;
    }
    public function codepay($data,$typ,$uname)
    {
        $type=$typ==2?1:3;
        $payno = $data['balance_sn'];
        $money = $data['bpprice'];
        //$uname=$data['username'];
        // dump($data);die;
		header('Location:'."/codepay/pay.php?type=$type&money=$money&user=$uname");
		exit;
    }
    ...
    {
        //软件接口配置
		$key_="i";//接口KEY  自己修改下 软件上和这个设置一样就行
		$md5key="i";//MD5加密字符串 自己修改下 软件上和这个设置一样就行
    ...
    public function mcbpay()
    {
        ...
        //软件接口配置
        $key_="333";//接口KEY  自己修改下 软件上和这个设置一样就行
        $md5key="333";//MD5加密字符串 自己修改下 软件上和这个设置一样就行
    //软件接口地址 http://域名/mcbpay/apipay.php?payno=#name&tno=#tno&money=#money&sign=#sign&key=接口KEY
