	...
	public function order()
	{
		...
		foreach ($orderlist as $k => $v) {
			...
			if (isset($dang_kong) && ($dang_kong == 1 || $dang_kong == 2 || $dang_kong == 3 || $dang_kong == 4)) {
			    ...
			    //盈利
			    if ($dang_kong == 1) {
			        $yingli = $v['fee']*($randUp/100);
                    $d_map['is_win'] = 1;
                } else if ($dang_kong == 2) {
                    $yingli = -1*$v['fee']*$randDown/100;
                    $d_map['is_win'] = 2;
                }else if ($dang_kong == 3) {//全赢
                	$num = $risk['min_qloss'] + mt_rand() / mt_getrandmax() * ($risk['max_qloss']-$risk['min_qloss']);
        			$randDown= sprintf("%.1f", $num);
                    $yingli = 1*$v['fee']*$randDown/100;
                    $d_map['is_win'] = 1;
                }else if ($dang_kong == 4) {//全亏
                	$num = $risk['min_qloss'] + mt_rand() / mt_getrandmax() * ($risk['max_qloss']-$risk['min_qloss']);
        			$randDown= sprintf("%.1f", $num);
                    $yingli = -1*$v['fee']*$randDown/100;
                    $d_map['is_win'] = 2;
