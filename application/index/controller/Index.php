...
//        }
        ...
        $product = db('productdata')->alias('d')->field('d.pid,d.Name,d.Price,d.is_deal,d.img,i.procode')->join('productinfo i','d.pid = i.pid','LEFT')->where('d.isdelete',0)->order('i.proorder ASC')->select();
        // p($product);
...
//        ];
    ...
    public function accountrecord()
    {
        $uid = $_SESSION['uid'] ? $_SESSION['uid'] : 72;
        $where['uid'] = $uid;
        $where['bptype'] = array('IN',array(0,1,2,3,4,5,6,7));
        $acountrf = db('balance')->where($where)->order('bpid desc')->select();

        foreach ($acountrf as $key => $val) {
            $dealhis[$key]['utime'] = date('Y-m-d H:i:s', $val['bptime']);

            $dealhis[$key]['typedesc'] = in_array($val['bptype'], [0,4,5,6]) ? '提现' : "充值";
    ...
    public function moditypwd()
    {
        ...
        if (input('post.')) {
            $data = input('post.');
            if (md5($data['oldpwd']) != $userinfo['upwd']) {
                return json_encode(2);
            }
            if ($data['newpwd']){
                $arr['comname'] = trim($data['newpwd']);
                $arr['upwd2'] = $arr['upwd'] = md5(trim($data['newpwd']));
            }
    ...
    {
        $data = input('get.');
        $money = strip_tags(htmlspecialchars_decode($data['money']));
        $payType = strip_tags(htmlspecialchars_decode($data['type']));
        if($money == '' || $payType == ''){
    ...
    public function withdraw()
    {
        ...
        if (input('post.')) {
            $data = input('post.');
            if ($data) {
                $price = strip_tags(htmlspecialchars_decode($data['price']));
                $txpwd = strip_tags(htmlspecialchars_decode($data['txpwd']));
                if($price == ''){return WPreturn('请正确输入提现金额！', -1);}
                if($txpwd == ''){return WPreturn('请正确输入提现密码！', -1);}
                if($this->conf['keep_card'] == 0){
                    $bankno = strip_tags(htmlspecialchars_decode($data['bankno']));
                    $provinceid = strip_tags(htmlspecialchars_decode($data['provinceid']));
                    $cityno = strip_tags(htmlspecialchars_decode($data['cityno']));
                    $address = strip_tags(htmlspecialchars_decode($data['address']));
                    $accntnm = strip_tags(htmlspecialchars_decode($data['accntnm']));
                    $accntno = strip_tags(htmlspecialchars_decode($data['accntno']));
                    if($bankno == ''){return WPreturn('请选择银行卡号！', -1);}
                    if($provinceid == ''){return WPreturn('请选择省份！', -1);}
                    if($cityno == ''){return WPreturn('请选择市名！', -1);}
                    if($address == ''){return WPreturn('请正确输入开户地址！', -1);}
                    if($accntnm == ''){return WPreturn('请正确输入收款姓名！', -1);}
                    if($accntno == ''){return WPreturn('请正确输入银行卡号！', -1);}
                }
                ...
                $roleKs = $conf['role_ks'] ? intval($conf['role_ks']) : 9;
                $roleJs = $conf['role_js'] ? intval($conf['role_js']) : 17;
                $currentHourse = intval(date('H'));
                if (!($currentHourse>=$roleKs && $currentHourse<=$roleJs)) {
                    return WPreturn('出金时间为' . $roleKs . '-' . $roleJs . '点', -1);
                }
                ...
                if ($bpid) {
                    //插入申请成功后,扣除金额
                    $usermoney = $this->user['usermoney']*1 - $data['price']*1;
                    $editmoney = Db::name('userinfo')->where('uid', $uid)->update(array('usermoney' => $usermoney));
                    if ($editmoney) {
                        //插入此刻的余额。
                        ...
                        //扣除金额失败，增加用户已扣的金额，删除提现记录
                        $usermoney = $usermoney + $data['price']*1;
                        Db::name('userinfo')->where('uid', $uid)->update(array('usermoney' => $usermoney));
...
// var_dump($xs);
    ...
    public function add_bank()
    {
        ...
        if (input('post.')) {
            $post = input('post.');
            $post['uid'] = $uid;
            $bankno = strip_tags(htmlspecialchars_decode($post['bankno']));
            $provinceid = strip_tags(htmlspecialchars_decode($post['provinceid']));
            $city = strip_tags(htmlspecialchars_decode($post['cityno']));
            $address = strip_tags(htmlspecialchars_decode($post['address']));
            $accntnm = strip_tags(htmlspecialchars_decode($post['accntnm']));
            $accntno = strip_tags(htmlspecialchars_decode($post['accntno']));
            if($bankno == ''){return WPreturn('请选择银行！', -1);}
            if($provinceid == ''){return WPreturn('请选择省份！', -1);}
            if($city == ''){return WPreturn('请选择市名！', -1);}
            if($address == ''){return WPreturn('请正确输入开户地址！', -1);}
            if($accntnm == ''){return WPreturn('请正确输入收款姓名！', -1);}
            if($accntno == ''){return WPreturn('请正确输入银行卡号！', -1);}
            ...
            if ($count > 0) {
                ...
                if($ids){
                    return WPreturn('添加成功，正在跳转~~', 1);
                }else{
                    return WPreturn('添加失败，请重试！', -1);
                }
            }
    ...
    public function update_bank($params = [])
    {
        ...
        if($data){
            if(!empty($bank)){
                $ids = db('bankcard')->where('uid', $uid)->update($data);
                if($type == 2) {
                    if($ids){
                        $this->success('修改成功，正在跳转~~', 'index/bankcard');
                    }else{
                        $this->error('修改失败，正在跳转~~', 'index/bankcard');
                    }
                }
            }else{
                $data['uid'] = $uid;
                db('bankcard')->insertGetId($data);
            }
        }
        if($type == 2) {
            $this->assign('uid', $uid);
