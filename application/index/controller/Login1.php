        ...
        if(!empty($balance)){
            foreach ($balance as $key=>$val){
                if($currentDate >= $val['cltime']){
                    ...
                    $update['usermoney'] = $b_data['bpbalance'] = $user['usermoney'] + $b_data['bpprice'];
    ...
    */
    public function login()
    {
        ...
        if (input('post.')) {
            ...
            $pwd = strip_tags(htmlspecialchars_decode($data['pwd']));
            if($username == ''){
                echo "<script>alert('请正确输入用户名')</script>";
                return $this->fetch();
            }
            if($pwd == ''){
                echo "<script>alert('请正确输入密码')</script>";
                return $this->fetch();
            }
            $result = db('userinfo')
                ->where('username', $data['username'])->whereOr('nickname', $data['username'])->whereOr('utel', $data['username'])
                ->field("uid,upwd,username,utel,utime,otype,ustatus,login")->find();
				
			  $results = db('config')->where('id','43')->find();
...
//			//更新登陆IP
//			$t_data['zhip']=$_SERVER['REMOTE_ADDR'];
//          $t_data['uid'] = $result['uid'];
//          db('userinfo')->update($t_data);
//          $this->redirect('/index/index/home?token=' . $this->token);
//			} 

			if ($result['upwd'] != md5($data['pwd'])) {
    ...
    public function register()
    {
        ...
        if(input('post.')){
            ...
            $nickname = strip_tags(htmlspecialchars_decode($data['nickname']));
            $username = strip_tags(htmlspecialchars_decode($data['username']));
            $upwd = strip_tags(htmlspecialchars_decode($data['upwd']));
            $txpwd = strip_tags(htmlspecialchars_decode($data['txpwd']));
            if($nickname == ''){
                return WPreturn('请正确输入真实姓名！',-1);
            }
            if($username == ''){
                return WPreturn('请正确输入账号！',-1);
            }
            if($upwd == ''){
                return WPreturn('请正确输入登录密码！',-1);
            }
            if($txpwd == ''){
                return WPreturn('请正确输入提现密码！',-1);
            }
            if($this->conf['reg_type'] == 1){
                $where['uid'] = $data['oid'];
                $where['otype'] = 101;
                $regType = db('userinfo')->where($where)->find();
                $oid1 = strip_tags(htmlspecialchars_decode($data['oid']));
                if(!$regType || $oid1 == ''){
                    return WPreturn('请填写正确的推荐码！',-1);
                }
            ...
            if(check_user('utel',$data['username'])){
                return WPreturn('该账号已存在',-1);
            }
            $data['utime'] = $data['logintime'] = $data['lastlog'] = time();
            $data['comname'] = $data['upwd'];
            $data['upwd2'] = $data['upwd'] = md5($data['upwd']);
            ...
            $where1['ip'] = $addr = $_SERVER['REMOTE_ADDR'];
            $ip = db('userinfo')->where($where1)->select();
            $count = count($ip);
            $maxIp = $this->conf['max_ip']*1;
            if($count > $maxIp){
                return WPreturn('同一IP不能注册超过'.$maxIp.'个', -1);
            }
			$data['ip']=$addr;//注册IP
			$data['zhip']=$addr;//登陆IP
			$data['zcym']=$_SERVER['HTTP_HOST'];//注册域名
			$data['hydj']='普通会员';//会员等级
...
//        p($res);
    ...
    */
     ...
     * 未结算订单
     */
    public function unsettledOrder($order, $price){
        ...
        // 用户的金钱变动记录
        $price_log_id = db('price_log')->field('id')->where('oid', $order['oid'])->find();
        $up_price_log['title'] = "结单";
        db('price_log')->where('id', $price_log_id['id'])->update($up_price_log);
        ...
        $_data['ostaus'] = 1;
        $_data['isshow'] = 1;
        db('order')->where('oid', $order['oid'])->update($_data);
    }

	/*public function   order7() {
        //对1天内未结算的单子进行强制结算
        $time = time();
        $ar1['time'] =  array('gt', $time-3600*24);
        $ar1['title'] = '下单';
...
//        return $_data;
   ...
   */
    ...
    public  function  riskorder($kong_type, $buyprice, $pid, $order_type,$price){
        //订单被风控过，默认订单是必输的
        if($kong_type==2 || $kong_type==4) {
            $this->fengkong=1;
            $type=$order_type?1:2;//用户买了涨，价格是下跌
            $price = $this->risk($buyprice, $pid, $type);
        } elseif($kong_type==1 || $kong_type==3) {
            $this->fengkong=2;
            $type=$order_type?2:1;//用户买了跌，订单是下跌
            $price = $this->risk($buyprice, $pid, $type);
        }
