...
function hold_order_list() {
    ...
    $.get(url,function(resdata){
       ...
       if(resorderlist){
            ...
            timer_orderlist = setInterval("show_order_list122()",1000);
...
function show_order_list122() {
    ...
    $.each(resorderlist,function(k,v){
        ...
        //亏损随机数
        var randDown = (Math.random() * (v.lt - v.lb) + v.lb).toFixed(2);
        //盈利随机数
        var randUp = (Math.random() * (v.wt - v.wb) + v.wb).toFixed(2);

        var timestamp = Date.parse(new Date());
        if(typeof(v.selltime)=="undefined"){v.selltime = 0}
        //var  _end_time = (v.selltime*1 - _ftime*1);
        var  _end_time = (v.endprofit - ts);
        ...
        if(_end_time >= 0){
            ...
            if(v.ostyle == 0){
                ...
                var ostyle_name = "买涨";
                if(chaprice >0){
                    closeprice = v.fee*randUp/100;
                    closeprice_class = 'in_money';
                }else if(chaprice <0){
                    closeprice = v.fee*(-1)*randDown/100;
