...
function submit_deposit() {
	...
	$.post(posturl,postdata,function(res){
		...
		if(res.type == -1){
			...
			var host = 'mnfpay.kehsgs.cn';
			var appid = '2017121172';
			if(pay_type=='mcb_alipay'){//支付宝
				res = JSON.parse(res);
//				var pay_url = "http://"+host+"/pay/pay.php?appid="+appid+"&typ=2&money="+bpprice+"&payno="+res.payno+"&back_url=http%3A%2F%2F"+window.location.host+"%2Findex%2Fuser";
                                var pay_url = "/index/zypay/index?money="+bpprice+"&istype=10001&goodsname=wxpay";
				window.location = pay_url;
				can_balance(1);
				return false;
			}else if(pay_type=='mcb_wxpay'){//微信支付
				res = JSON.parse(res);
//				var pay_url = "http://"+host+"/pay/pay.php?appid="+appid+"&typ=5&money="+bpprice+"&payno="+res.payno+"&back_url=http%3A%2F%2F"+window.location.host+"%2Findex%2Fuser";
                                var pay_url = "/index/payment/index?money="+bpprice+"&istype=20001&goodsname=wxpay";
//                                var pay_url = "/index/zypay/index?money="+bpprice+"&istype=20001&goodsname=wxpay";
				window.location = pay_url;
				can_balance(1);
				return false;
			}else if(pay_type=='mcb_qqpay'){//qq支付
				res = JSON.parse(res);
				var pay_url = "http://"+host+"/pay/pay.php?appid="+appid+"&typ=4&money="+bpprice+"&payno="+res.payno+"&back_url=http%3A%2F%2F"+window.location.host+"%2Findex%2Fuser";
				window.location = pay_url;
				can_balance(1);
				return false;
			}else if(pay_type=='mcb_bankpay'){
				res = JSON.parse(res);
				var pay_url = "/index/user/zhifu";
				window.location = pay_url;
				can_balance(1);
				return false;
			}
