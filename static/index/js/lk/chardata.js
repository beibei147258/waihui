...
function endtimes() {
    ...
    var yuce_case = 0;
    //亏损随机数
    var randDown = (Math.random() * (order_data.lt - order_data.lb) + order_data.lb).toFixed(2);
    //盈利随机数
    var randUp = (Math.random() * (order_data.wt - order_data.wb) + order_data.wb).toFixed(2);
    //股票涨
    if(order_data.buyprice*10 < newprice*10){
        ...
        if(order_data.ostyle == 0){     //买涨
            yuce_case = '+'+(order_data.fee * (1 + randUp / 100)).toFixed(2);
            $('.yuce').removeClass('fall');
            $('.yuce').addClass('rise');
        }else{
            yuce_case = (order_data.fee * (1 - randDown / 100)).toFixed(2);
...
function get_this_order() {
    ...
    $.get(tourl,function(resdata){
        if(resdata){
            ...
            ccout=setTimeout("getonedata()",100);
